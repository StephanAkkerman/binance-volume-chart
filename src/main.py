...
# This list keeps track of the top tickers based on volume
...
# Sets list of top coins on Binance, based on volume
def top_volume():
...
def change_assets():
    ...
    symbol_list = list(sorted_volume.head(number))
    ...
    try:
        ...
        for k in crypto_data:
            ...
            if t < t2:
                ...
                # Add it all together
                data = [t] + [close] + [diff]
                candle = pd.DataFrame([data], columns="Time Close Diff".split()).astype(
                    {"Time": "datetime64[ms]"}
                )
...
def make_plot(ax, timeframe):
    ...
    for symbol in symbol_list:
        ...
        try:
            if timeframe == "1 day":
                hist_candles = client.get_klines(
                    symbol=symbol, interval="5m", limit=288
                )
            if timeframe == "1 week":
                hist_candles = client.get_klines(
            ...
            if timeframe == "1 month":
                hist_candles = client.get_klines(
                    symbol=symbol, interval="4h", limit=186
                )
            if timeframe == "1 year":
                hist_candles = client.get_klines(
                    symbol=symbol, interval="1d", limit=365
                )
        except Exception as e:
            print(symbol)
    ...
    for symbol in sorted_dict:
        df = symbol_dict[symbol]
        plots[symbol] = fplt.plot(
            df["Diff"], ax=ax, legend=symbol, color=color_list[counter]
        )
        counter += 1
